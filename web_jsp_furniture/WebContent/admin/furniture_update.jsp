		...
		<div id="b_right">
			...
			<table class="boardForm">
				...
				<tbody>
					<tr>
						<th>제품명</th>
						<td colspan="3"><input type="text" name="t_p_name" value="<%=dto.getP_name()%>" style="width:400px;" class="input600"></td>
					</tr>	
					<tr>
						<th>사이즈</th>
						<td colspan="3"><input type="text" name="t_p_size" value="<%=dto.getP_size()%>" style="width:400px;" class="input600"></td>
					</tr>	
					<tr>
						<th>정렬순서</th>
						<td colspan="3">
						<input name="t_ori_img_order" type="hidden" value="<%=dto.getImg_order()%>">
						<select name="t_img_order">
							<option <%if(dto.getImg_order().equals("7")){out.print("selected");} %> value="7">없음</option>
							<option <%if(dto.getImg_order().equals("1")){out.print("selected");} %> value="1">1</option>
							<option <%if(dto.getImg_order().equals("2")){out.print("selected");} %> value="2">2</option>
							<option <%if(dto.getImg_order().equals("3")){out.print("selected");} %> value="3">3</option>
							<option <%if(dto.getImg_order().equals("4")){out.print("selected");} %> value="4">4</option>
							<option <%if(dto.getImg_order().equals("5")){out.print("selected");} %> value="5">5</option>
							<option <%if(dto.getImg_order().equals("6")){out.print("selected");} %> value="6">6</option>
						</select>
						</td>
					</tr>	
					<tr>
						<th>Attach</th>
						<td colspan="3">
						<p><%=dto.getAttach()%><input type="hidden" name="t_ori_attach" value="<%=dto.getAttach()%>"></p>
						<input name="t_attach" type="file" class="input600"></td>
					</tr>	
					<tr>
						<th>Writer</th>
						<td><%=sessionName%></td>
...
if(file.value !=""){
	...
	if(fileSize > maxSize){
		...
		furniture.action="db_furniture_update.jsp";
		furniture.submit();
	}
</script>
